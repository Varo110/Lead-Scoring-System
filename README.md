# 🎯 B2B Intelligent Lead Scoring System

### 🔗 [VER DEMO INTERACTIVA (Dashboard)](https://dashboard-lead-scoring.lovable.app)
*Nota: pegar documento de

---

## 💼 Resumen del Proyecto
Sistema diseñado para optimizar el tiempo de los equipos de ventas B2B. Utiliza un algoritmo de puntuación basado en datos para identificar qué leads tienen mayor probabilidad de compra, aumentando el **Revenue por Hora Comercial**.

## 🛠️ Tecnologías Usadas
* **Python:** Generación de datos sintéticos y motor de cálculo.
* **Pandas:** Procesamiento y limpieza de datos.
* **Lovable (React/AI):** Dashboard visual para el usuario final.

## 🧠 Cómo funciona el Scoring
El sistema puntúa cada lead de 0 a 100 basándose en:
1.  **Perfil (50%):** Cargo del contacto (C-Level vs Junior) y tamaño de la empresa.
2.  **Comportamiento (50%):** Si solicitó demo, descargó whitepapers o visitó la web.

> **Resultado:** Clasificación automática en **Hot**, **Warm** y **Cold** leads.

---
*Proyecto creado por Álvaro Pérez.*
